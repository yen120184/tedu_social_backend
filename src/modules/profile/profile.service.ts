...
class ProfileService {
  ...
  public addFriend = async (fromUserId: string, toUserId: string) => {
    ...
    if (
      ...
      throw new HttpException(
        400,
        "You has been already seed a friend request to this user"
      );
    }
  ...
  public acceptFriendRequest = async (
    ...
    if (!requestProfile) {
      throw new HttpException(400, "There is not profile for request user");
    }
    ...
    if (
      ...
      throw new HttpException(
        400,
        "Request user not has been already seed a friend request to you"
      );
    }
